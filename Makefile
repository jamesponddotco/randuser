...
doc: # Builds the manpage.
	$(SCDOC) <doc/randuser.1.scd >randuser.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 randuser $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 randuser.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
...
clean: # Cleans cache files from tests and deletes any build output.
	$(RM) -f cover.out randuser randuser.1
